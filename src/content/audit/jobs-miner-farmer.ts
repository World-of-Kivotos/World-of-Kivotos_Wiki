...
export const auditJobsMinerFarmer: AuditDoc = {
  id: 'jobs-miner-farmer',
  name: '矿工与农夫职业审计',
  ...
  facts: [
    ...
    { label: '职业 ID', value: 'miner / farmer' },
    { label: '总体状态', value: '已入包、已装配、玩家可达' },
    { label: '等级范围', value: 'L1-L10，共享累计经验曲线' },
    { label: '配置状态', value: '职业数值均为源码硬值，尚无 miner.* / farmer.* 服务端配置' },
  ],
  intro:
  ...
  sections: [
    {
      heading: '启动接线与可达性结论',
      table: {
        columns: ['链路', '当前 main 的实际接线', '结论'],
        monoCols: [0, 1],
        rows: [
          [
            'MiningDim',
            'registerSubsystems() 加入 JobFrameworkSystem、CombatSystem、MinerSystem、FarmerSystem；构造器逐个调用 register(modBus, forgeBus)',
            '四个子系统启动时都会装配',
          ],
          [
            'JobFrameworkSystem',
            '注入 JobServices、注册玩家职业能力与同步、/job、job.progress、admin.job.setLevel',
            '共享等级、经验、命令和 WebUI 服务可达',
          ],
          [
            'MinerSystem',
            '订阅 Forge 事件，注册矿工网络、HUD 状态与 job.miner.state / job.miner.scan',
            '在矿业维度实例区域内可实际游玩',
          ],
          [
            'FarmerSystem',
            '注册方块、物品、创造页、全局战利品修改器、事件、/farmer 与两个 WebUI action',
            '配方、种植、收获和出售均有玩家入口',
          ],
          [
            'EntryGateway',
            '进入 Easy / Medium / Hard 时读取 JobId.MINER，门槛分别为 L1 / L4 / L8',
            'MinerLevelGate 已被真实入口与 WebUI 入口共用',
          ],
        ],
      },
      note:
    ...
    },
    {
      heading: '共享等级、存档与经验',
      table: {
        columns: ['等级', '达到本级的累计有效 XP', '从本级升下一级'],
        numericCols: [0, 1, 2],
        rows: [
          ['1', '0', '3300'],
          ['2', '3300', '3800'],
    ...
    {
      heading: '农夫等级、耕地上限与产量',
      table: {
        columns: ['等级', '本级最高已解锁档', '全服跨维度放置上限'],
        numericCols: [0, 2],
        rows: [
          ['1', 'low', '9'],
          ['2', 'low', '12'],
    ...
    },
    {
      heading: '五档农夫耕地实测数值',
      table: {
        columns: ['tierId', '解锁', '期望成熟时间', '成熟产量倍率', '每块每小时', '每块 6 小时', '成熟破坏原始 XP'],
        monoCols: [0],
    ...
    },
    {
      heading: '农夫实际操作流程',
      steps: [
        '用 1 个 minecraft:wheat_seeds 无序合成 1 个 miningdim:farmer_seed。',
        '用 dirt 与 farmer_seed 合成 low 耕地；以后依次用铜锭、铁锭、金锭、钻石把上一档耕地升级到 medium、high、premium、supreme。',
        '达到对应农夫等级后放置耕地。玩家放置事件会先检查档位解锁，再检查本人全局上限，成功后把维度与坐标登记到主世界 SavedData。',
        '种植原生种子，或在该土壤上种受支持的原版、Farmer’s Delight 作物，等待自然随机刻成熟。',
        '破坏成熟作物获得原始职业 XP。原生作物额外由事件弹出 farmer_wheat；兼容作物由全局战利品修改器放大指定主产物。',
        '达到农夫 L2 后可用 /farmer sell <amount> 或 job.farmer.sell 出售背包中的 farmer_wheat。',
      ],
    },
    {
      heading: '农夫配方清单',
      ...
      note: '权威资源：src/main/resources/data/miningdim/recipes/farmer/*.json。',
    },
    {
      heading: '原生作物、兼容作物与 Farmer’s Delight',
      table: {
        columns: ['作物方块 ID', '会被倍率放大的产物 ID'],
        monoCols: [0, 1],
        rows: [
    ...
    },
    {
      heading: '矿工被动数值表',
      table: {
        columns: ['L', '挖速倍率', '省耐久', '矿脉时运额外期望', 'Danger 时间系数', '陷阱减伤'],
      ...
      note: '权威源：MinerConstants.java、MinerSkills.java、MinerSurvival.java、combat/PlayerDamageReduction.java、danger/Danger.java、pressure/MobPressureSystem.java。',
    },
    {
      heading: '矿工主动技能精确数值',
      table: {
        columns: ['L', '连锁池 / 整池回满', '矿探半径 / CD', '陷阱探测半径 / CD', '归途 CD', '隧道 CD', '声东击西 CD'],
        monoCols: [1, 2, 3, 4, 5, 6],
        rows: [
          ['1', '-', '-', '-', '-', '-', '-'],
          ['2', '16 / 6000t', '-', '-', '-', '-', '-'],
          ['3', '20 / 5775t', '6 / 6000t', '-', '-', '-', '-'],
      ...
      note: '所有整型插值最终使用 Java Math.round。权威源：MinerConstants.java 与 MinerSkills.java。',
    },
    {
      heading: '矿工按键、网络、HUD 与 WebUI',
      table: {
        columns: ['入口', '精确 ID 或契约'],
        monoCols: [0, 1],
        rows: [
    ...
    },
    {
      ...
      note: '权威源：ChainMiningEngine.java、MinerActions.java、MinerChargeState.java、AutoCollectSmelt.java、MinerSystem.java。',
    },
    {
      heading: '矿物与陷阱探测清单',
      table: {
        columns: ['解锁档', '单次选择优先序与精确方块范围'],
        monoCols: [1],
        rows: [
      ...
      note: '权威源：OreScanService.java、ore/OreType.java、TrapScanService.java、trap/TrapRegistry.java。',
    },
    {
      heading: '已确认缺陷、绕过面与接口漂移',
      table: {
        columns: ['级别', '对象', '当前事实'],
        rows: [
