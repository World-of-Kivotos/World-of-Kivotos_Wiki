...
export const auditGameplay: AuditDoc = {
  id: 'gameplay',
  name: '矿洞玩法与运行链路',
  ...
  facts: [
    { label: '审计快照', value: 'main@701093bd8492' },
    { label: '维度 ID', value: 'miningdim:mining' },
    { label: '实例模型', value: '每难度 1 个永久共享区域' },
    { label: '区域默认尺寸', value: '256 x 192 x 256' },
    { label: '实际生成器', value: 'minecraft:noise' },
    { label: '动态陷阱', value: '已入包但调度不可达' },
  ],
  intro:
  ...
  sections: [
    {
      heading: '玩家实际可走的完整循环',
      steps: [
        '在主世界右键或踩上 miningdim:entrance_easy、miningdim:entrance_medium、miningdim:entrance_hard，或者执行 /mining enter <easy|medium|hard>；WebUI 的 mining.enter 也汇入同一个 EntryGateway。',
        '服务端检查矿工职业等级与该难度的 CREDIT 入场费。默认三档费用均为 0；门槛分别为矿工 L1、L4、L8。',
        'InstanceManager 把请求路由到该难度唯一的永久共享实例，不创建个人副本，也不按队伍分配新图。',
        'EntryGateway 最多等待实例就绪 200 tick，再强加载区域中心 3 x 3 区块并最多等待 FULL 状态 200 tick。',
        'SpawnSystem 从安全点池选点；池不足时在 Y=49 附近建 3 x 3 石头平台并清理头顶空间。入场后有 200 tick 出生压力冻结。',
        '区块按玩家位置维持默认 9 x 9 加载窗，其中 5 x 5 为 ticking；窗口始终裁在所属 region 内。',
        '玩家挖原版矿和能源矿；静态伪装矿可能触发爆炸、岩浆或落砂。压力 danger 随难度与停留时间上升，显式刷怪链仍可运行。',
        '执行 /mining leave 或 WebUI mining.leave 回到进入前记录的维度与方块坐标。强制重置会先撤离在场玩家。',
        '重置不会在原地清空重生，而是把该难度的 region 横向滑到未生成过的新坐标；新区块之后仍由 minecraft:noise 按需生成，旧区块进入分批磁盘回收队列。',
      ],
      note: '“动态陷阱”和“定时自动重置”虽然都有完整业务类，但当前调度器存在 Long.MIN_VALUE 溢出，实际首轮永远不能通过；不能写成玩家可用功能。',
    },
    {
      heading: '启动接线与单一权威链',
      table: {
        columns: ['顺序', '子系统', '当前职责与可达性'],
        monoCols: [1],
        rows: [
    ...
    {
      heading: '默认 region 几何与边界',
      table: {
        columns: ['区域', '难度序号', '初始 X', '初始 Z', 'Y', '群系'],
        numericCols: [1],
        monoCols: [5],
        rows: [
    ...
    },
    {
      heading: '三档地形与资源构成',
      table: {
        columns: ['难度', '地下结构与杂项', '原版矿 placed feature', '自定义矿', '静态陷阱'],
        monoCols: [2, 3, 4],
        rows: [
    ...
    },
    {
      heading: '原版矿物的真实参数',
      table: {
        columns: ['placed feature', '使用难度', '矿脉与暴露丢弃', '频率', '高度分布'],
        monoCols: [0],
        rows: [
      ...
      note: '维度顶部只有 Y=127，但 Easy 仍引用 136..top 的 coal_upper，实际采样点在建筑高度外；iron_upper 大量采样也高于顶部。Hard 的 redstone_lower 与 diamond 三项又有大量采样低于 Y=-64。它们不是按本维度 192 高重新标定的分布。',
    },
    {
      heading: '自定义矿物与相对高度',
      table: {
        columns: ['configured feature', '方块 ID', 'size / discard', 'placed feature 与次数'],
        monoCols: [0, 1, 3],
        rows: [
    ...
    },
    {
      heading: '入场门、费用与入口方块',
      table: {
        columns: ['难度', '入口方块', '矿工等级', '默认费用', '默认浮空字'],
        numericCols: [2, 3],
        monoCols: [1],
        rows: [
    ...
    },
    {
      ...
      note: 'spawn.avoidTrapZones=true 与 spawn.mustBeMainComponent=true 都没有代码读取；安全点没有查询 TrapRegistry，也没有连通分量数据，因此配置名表达的两项保护并未生效。',
    },
    {
      heading: '静态伪装陷阱',
      table: {
        columns: ['ID', '次数/区块', '难度', '反应窗口', '实际效果'],
        monoCols: [0],
        rows: [
    ...
    {
      heading: '动态陷阱代码与真实可达性',
      table: {
        columns: ['类型', '代码门槛与冷却', '代码效果', 'main 实际状态'],
        rows: [
    ...
    },
    {
      ...
      table: {
        columns: ['档位', 'danger', '间隔', '波次', '怪物池', '压暗系数'],
        monoCols: [4],
        rows: [
          ['SAFE', '[0,0.20)', '不刷', '0', '-', '0'],
    ...
    },
    {
      ...
      note: 'DynamicTrapEngine 自己已有 cooldownAllows(Long.MIN_VALUE, ...) 的正确哨兵处理和回归测试，但 TrapSystem 外层、AutoResetScheduler 与 MobPressureSystem 没有复用该判据。',
    },
    {
      heading: '主动撤离、断线与死亡',
      table: {
        columns: ['路径', '当前行为', '限制或缺口'],
        rows: [
    ...
    },
    {
      ...
      note: '这是运行可达性缺陷，不是“服务器尚未等够 2/4/6 小时”。页面若展示 nextResetGameTime，只能视作配置推算，不能视作调度器会兑现的承诺。',
    },
    {
      heading: '维度内规则与隔离范围',
      table: {
        columns: ['规则', '默认值/判据', '覆盖范围'],
        monoCols: [1],
        rows: [
    ...
    },
    {
      ...
      note: '有效命令没有二次确认，也没有执行 reset.cooldownSeconds=300 或 confirmationWindowSeconds=15。破坏性重置的服务端确认与冷却只存在于未装配旧命令树。',
    },
    {
      heading: '有效 WebUI action 与未装配旧命令',
      table: {
        columns: ['入口', '状态', '说明'],
        monoCols: [0],
        rows: [
    ...
    {
      heading: '基础 registry：方块、物品与创造栏',
      table: {
        columns: ['注册 ID', '类型/属性', '物品与玩家入口', '审计状态'],
        monoCols: [0],
        rows: [
    ...
    },
    {
      heading: '配置项实际生效矩阵',
      table: {
        columns: ['配置组', '确实生效', '无效、部分生效或被缺陷阻断'],
        monoCols: [1, 2],
        rows: [
      ...
      note: 'ore.useDatapackDistribution=false 不会切换到旧内置表；现行世界生成仍照常读取 JAR 内数据包。不要把这些键当成在线调矿开关。',
    },
    {
      heading: '可达代码、死代码与缺失玩法',
      table: {
        columns: ['对象', '物理状态', '结论'],
        rows: [
    ...
    },
    {
      heading: '复核源码入口',
      code: {
        caption: '相对 D:/Repo/Wok-Project 的关键证据路径',
        text: [
