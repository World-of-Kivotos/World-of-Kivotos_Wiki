...
name: main 功能审计
en: Temporary audit
tagline: 只读统计当前生产 JAR 的功能、职业、注册物、联动与完整玩法，用于版本核对和 Bug 复现。
facts:
  - label: 快照
    value: main@701093bd
  - label: 生产 JAR
...
> 该区域当前公开可访问，不包含服务器凭据、玩家数据或内部密钥。

## 阅读顺序

1. 先看“审计口径与构建证据”，确认版本和状态标签。
2. 按玩法、职业、方块物品、联动等专题查具体功能。
3. 遇到“条件可用”或“仅骨架/不可达”时，按页内配置门和前置条件复现。
4. Bug 回报附上页面中的注册 ID、main 提交和状态标签，避免把旧分支行为混进来。
