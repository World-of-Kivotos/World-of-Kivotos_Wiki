...
export const auditJobsChefEngineer: AuditDoc = {
  id: 'jobs-chef-engineer',
  name: '厨师与铸甲师完整审计',
  en: 'Chef / Engineer audit',
  group: '职业',
  tagline: '核对两个职业的注册链、完整操作、物品与精确默认数值，并区分已接线玩法和仅注册内容。',
  facts: [
    { label: '源码快照', value: 'main@701093bd8492' },
    { label: '职业 ID', value: 'chef / engineer' },
    { label: '注册方块', value: '5 张调味台 + 6 张生产台' },
    { label: '工程师装备', value: '6 修复板 + 54 板甲 + 21 等离子盾 ID' },
    { label: '审计状态', value: '启动接线已证实；部分内容生存不可达' },
  ],
  intro:
  ...
  sections: [
    {
      ...
      note: '铸甲师稳定 ID 是 engineer，同时兼容 armorer 和“铸甲师”别名；EngineerSystem.name() 返回 ArmorerSystem 是遗留字符串，不是第三个职业。',
    },
    {
      heading: '厨师注册物与真实入口',
      table: {
        columns: ['类型', '注册 ID 或入口', '状态'],
        monoCols: [1],
        rows: [
    ...
    },
    {
      heading: '厨师完整操作流程',
      steps: [
        '放入一份食物，可选放入一份属于 #miningdim:seasonings 的调料；开始后服务器记录操作员 UUID。',
        '火候阶段从 0 开始，每 tick 增加 2，最大 200；绿区固定为 120–160。点击一次锁定火候，超过 200 视为过火。',
        '调味阶段依次出现 4 个提示；每个有效 20 tick，提示之间间隔 25 tick。玩家可命中 0–4 次。',
        '综合分为 0.5 × 火候准确度 + 0.5 × 命中比例，再同时受调味台阶级和职业等级上限裁切。',
        '服务器结算时消耗一份食物和可选的一份调料，将品质、效果和厨师 UUID 写入 MiningChef NBT，并把成品送入启动者背包；背包满则掉落。',
      ],
      bullets: [
    ...
    },
    {
      heading: '厨师品质判定与经验',
      table: {
        columns: ['品质', '综合分门槛', '等级上限', '最多效果', '允许失败效果', '允许战斗效果', '原始 XP'],
        numericCols: [6],
        rows: [
      ...
      note: '最终品质取原始判定、台阶上限、等级上限三者最低值。效果数=min(品质最大数,max(1,命中数))，因此调味 0 命中仍会得到 1 个效果；效果不重复，战斗效果至多一个。',
    },
    {
      heading: '调料联动与抽取偏向',
      table: {
        columns: ['风味', '调料来源', '权重提高到 3 的效果'],
        monoCols: [1],
        rows: [
      ...
      note: 'Farmers Delight 与 Flavor Immersed Daily 条目在标签中均为 required:false；对应模组未安装时不会阻止数据包加载。未匹配效果的权重保持 1。',
    },
    {
      heading: '厨师正面效果精确默认值',
      table: {
        columns: ['效果', '可出现条件', 'Low / Medium / High / Extraordinary / Radiant'],
        rows: [
    ...
    },
    {
      heading: '厨师失败效果与生命周期',
      table: {
        columns: ['效果', 'Low / Medium / High', '实际行为'],
        rows: [
      ...
      note: '窗口效果均为内存态、刷新而不叠加；死亡、换维度、登出会清除，服务器重启也会丢失。',
    },
    {
      heading: '厨师已证实缺陷',
      table: {
        columns: ['严重度', '问题', '证据与影响'],
        rows: [
    ...
    },
    {
      ...
      note: '配方 JSON 实际位于 data/miningdim/recipes/production_table_*.json，不在 engineer 子目录。',
    },
    {
      heading: '修复板生产与修复数值',
      table: {
        columns: ['阶级', '解锁等级', '输入', '基础产量', '最低时间', '原始 XP', '修复量'],
        numericCols: [1, 3, 4, 5],
        rows: [
    ...
    {
      heading: '铸甲师生产操作、权限与经验',
      steps: [
        '放置生产台时记录主人；主人可潜行空手切换锁。锁定后仅主人或权限等级 2 的管理员能打开。',
        '未锁时他人能开菜单并移动输入，但启动生产和取出产物仍要求主人或管理员。漏斗只接入输入槽，输出必须手取。',
        '服务器校验材料、职业等级、台阶上限、输出槽和 operator UUID 后开始校准。',
        '默认校准条宽 200、游标每 tick 移动 6、绿区宽 30；每个半程只可点击一次，命中加 20 进度与 1 qualityHit，未命中加 2，目标 100；每次触边重随机绿区。',
        '进度达到 100 且最低时间结束后产出；qualityHits 不少于 4 时独立有 50% 概率多出 1 个修复板。',
        '生产 XP 只在生产者取走输出时结算一次：原始 XP × (1+0.05×qualityHits) × 实际取出数量，再过共享日衰减。',
      ],
      note: '代码用生产台自身阶级的 produceTicks，而不是所选修复板阶级；在高级台制造低阶板会被高级台更长的最低时间拖慢。',
    },
    {
      heading: '纳米修复流程与效果抽取',
      bullets: [
    ...
    },
    {
      ...
      note: '#miningdim:bypasses_nano_shield 与等离子盾绕过标签均包含 #minecraft:bypasses_invulnerability、starve、drown、in_wall、cramming。',
    },
    {
      heading: '板甲伤害模型',
      bullets: [
    ...
    },
    {
      heading: '板甲六级基础矩阵',
      table: {
        columns: ['等级', 'R light / medium / heavy', 'Q light / medium / heavy', 'G light / medium / heavy', 'T light / medium / heavy'],
        rows: [
      ...
      note: '基础移动速度 light +10%、medium 0、heavy -12%。',
    },
    {
      heading: '板甲材料修正',
      table: {
        columns: ['材料', '耐久', 'R 泄漏乘数', 'Q 泄漏乘数', 'G 泄漏乘数', 'T 乘数', '额外移速惩罚'],
      ...
      note: '最终 R/Q/G=max(0,1-(1-base)×泄漏乘数)，但基础值为 0 时强制为 0；最终 T=基础 T×材料乘数；最终移速=体型基础-材料惩罚。',
    },
    {
      heading: '54 件板甲注册清单',
      table: {
        columns: ['等级', '注册 ID 后缀（体型 / 材料）'],
        monoCols: [1],
        rows: [
      ...
      note: '每个完整 ID 都是 miningdim:plate_armor_<后缀>。54 套物品模型、物品贴图和 layer_1 纹理均存在；49 个 Java 模型类是因为部分换色款共享几何，映射已穷举 54 项。',
    },
    {
      heading: '等离子盾 18 型精确默认值',
      table: {
        columns: ['型号', '当前盾容量', '总电量', '每伤害热量', '每秒散热', '每秒回盾', '回盾延迟 tick', '移速'],
        numericCols: [1, 2, 3, 4, 5, 6, 7],
        rows: [
          ['NANO I', '30', '60', '.50', '30', '18', '90', '0'], ['NANO II', '45', '84', '.44', '34', '22', '88', '0'], ['NANO III', '65', '114', '.39', '38', '26', '86', '0'],
          ['NANO IV', '90', '150', '.34', '42', '30', '84', '0'], ['NANO V', '120', '192', '.30', '46', '34', '82', '0'], ['NANO VI', '155', '240', '.26', '50', '38', '80', '0'],
          ['STANDARD I', '45', '112', '2.20', '10', '7', '110', '0'], ['STANDARD II', '70', '160', '2.00', '11', '8', '108', '0'], ['STANDARD III', '100', '216', '1.80', '12', '9', '106', '0'],
          ['STANDARD IV', '140', '280', '1.60', '13', '10', '104', '0'], ['STANDARD V', '190', '360', '1.40', '14', '11', '102', '0'], ['STANDARD VI', '250', '448', '1.20', '15', '12', '100', '0'],
          ['QUANTUM I', '65', '240', '.65', '5', '3', '130', '-12%'], ['QUANTUM II', '100', '336', '.58', '5.6', '3.5', '128', '-11.4%'], ['QUANTUM III', '150', '444', '.52', '6.2', '4', '126', '-10.8%'],
          ['QUANTUM IV', '215', '576', '.46', '6.8', '4.5', '124', '-10.2%'], ['QUANTUM V', '300', '732', '.41', '7.4', '5', '122', '-9.6%'], ['QUANTUM VI', '400', '912', '.36', '8', '5.5', '120', '-9%'],
        ],
      },
      note: '旧 ID plasma_shield_nano、plasma_shield_light、plasma_shield_heavy_ion 分别复用 NANO I、STANDARD I、QUANTUM I 数值。',
    },
    {
      heading: '等离子盾运行模型',
      bullets: [
    ...
    },
    {
      ...
      note: 'ChefGameTests、ChefWebUiGameTests、EngineerGameTests、EngineerWebUiGameTests、PlateArmorGameTests、PlasmaShieldGameTests 的 class 已入构建物；本轮遵守只读要求，没有执行会写构建目录的测试，不能把“测试代码存在”写成“测试已通过”。',
    },
  ],
}
