...
facts:
  - label: 职业 ID
    value: agent / brewer
  - label: 启动状态
    value: 均已由 MiningDim 装配
  - label: Agent 入口
    value: MCEF WebUI
  - label: Brewer 入口
    value: 原生方块菜单 + WebUI 状态页
  - label: 专属配置
    value: Agent 无 / Brewer 有
  - label: 审计快照
...
| 职业 | 生产注册链 | 已注册入口 | 实际结论 |
| --- | --- | --- | --- |
| 特勤干员 | MiningDim -> JobFrameworkSystem -> ChampionSystem -> AgentSystem -> AgentIntegrationBootstrap | job.agent.state / scan / seal | 扫描、封印、击杀 XP、额外信用点和精英增伤已接线 |
| 酿酒师 | MiningDim -> JobFrameworkSystem -> BrewerSystem | 物品、方块、BE、Menu、Screen、事件、配置、job.brewer.state | 生产、陈酿、饮酒和一条命永久层已接线 |
...
| 等级 | 扫描半径 | 扫描 CD | 解密范围 | 被动封印 窗口/CD | 机制封印 窗口/CD |
| ---: | ---: | --- | --- | --- | --- |
...
| 等级 | 可封最高星 | 槽位 | 信用点倍率 | activeAgent 对精英增伤 | 日/周悬赏槽预览 |
| ---: | --- | --- | --- | --- | --- |
| 1 | 不可封 | 0 | 1.00 | 5% | 1/0 |
| 2 | 不可封 | 0 | 1.25 | 6% | 1/0 |
| 3 | 3 | 1 | 1.50 | 7% | 2/0 |
...
> 日/周槽、最大悬赏星级、L4 周常和 L8 世界 Boss 权限只是现有面板预览表，不代表悬赏玩法已开放。
...
1. 从 /jobs/agent 打开 AgentPanel，调用 job.agent.scan；请求体不接受玩家自报半径或 CD。
2. 服务端在当前维度构建最多 8 个目标的个人快照；快照有效期等于本次扫描 CD，只存在进程内存。
3. 面板显示目标实体 ID、星级、距离、名称和已按等级解密的可封词条；L8 起额外返回精确方块坐标。
4. 玩家选择快照中的目标和已解密词条，调用 job.agent.seal。服务端再次校验目标、分类、星级、槽位、重复项和分类 CD。
5. 成功后从 MiningChampionData 真正移除词条，保存词条品质和维度，到期再恢复；SPRINT、OVERDRIVE、SELF_REPAIR 还会移除对应持续属性修饰。
...
| 分类 | 精确枚举 ID |
| --- | --- |
| 被动，可封 15 个 | BURNING, ARMOR_PIERCING, REND, HEAVY_CANNON, CORROSIVE, DOUBLE_STRIKE, QUADRUPLE_STRIKE, BLOODLUST, CHAOS_STRIKE, FROST, SPRINT, OVERDRIVE, BLINK, TACTICAL_BLINK, PHASE_WALK |
| 机制，可封 10 个 | ELECTRO_CHARGE, THUNDER, LITTLE_BOY, DEATH_MARK, VISUAL_DISRUPTION, SELF_REPAIR, COUNTER_UNIT, CAESAR_SWAP, BLADE_WALTZ, SUMMON_SUPPORT |
| 生存，不可封 10 个 | COMPOSITE_ARMOR, UHMWPE_ARMOR, HEAVY_ARMOR, REGEN_TISSUE, FLAMMABLE_REGEN, DEFLECTOR_SHIELD, FORTITUDE_SHIELD, THORNS, GIGANTISM, MINIATURIZATION |
:::

> 协议实际使用 AffixDef.name() 的大写枚举名，不是 namespaced ID；部分旧注释写法不准确。

## Agent 名义技能与真实 payload 的差距

| AgentScanField 宣称等级 | 设计字段 | 当前生产状态 |
| --- | --- | --- |
...
> 当前实际显示只有目标、星级、距离、实体类型/名称、分级解密的词条，以及 L8 后坐标。这些名义字段多由表格和 GameTest 锁定，但没有生产数据与前端渲染。
...
> 配方按聚合后的物品种类和数量精确相等匹配；少放、多放或混入异物均不启动。所有“小麦”均为 miningdim:farmer_wheat，原版小麦不能替代。

## 酿酒台实际流程

1. 玩家打开酿酒台；仅在当前没有开工时记录该玩家为操作者。
2. 在 5 个输入槽放入一条精确配方。开始时锁定酒种、操作者和按操作者 Brewer 等级掷出的品质。
3. 服务端推进 2,400 tick，约 120 秒；中途改变任意输入会中断并清空进度、锁定酒种和品质。输出满时暂停。
4. 完成后消耗材料并产出 1 瓶 vintage=0 的基酒。若输出同物品但 NBT 不同或放不下，剩余成品会掉在方块上方。
5. 完工时操作者在线才获得基础原始 XP 20；茅台永久层使其乘 1+0.10×层数，即 0 至 5 层为 20、22、24、26、28、30。离线仍产酒但不发 XP。

> 机器不暴露物品 capability，漏斗和外部机器不能自动输入/输出；所有等级均可使用全部配方，等级只影响品质概率。

## 品质系数与完整概率

| 等级 | LOW 1.0 | MID 1.5 | HIGH 2.0 | SUPERB 3.0 | BRILLIANT 5.0 |
| ---: | ---: | ---: | ---: | ---: | ---: |
| 1 | 62.50% | 37.50% | 0 | 0 | 0 |
...
> 酒窖也不暴露物品 capability，只支持手工操作。界面能看槽位、年份与变质状态，但没有精确的时间/燃料进度表。
...
> 层数保存在主世界 SavedData miningdim_brewer_buffs。只有真实、未被塔罗复活等机制取消的死亡会清层；所有永久效果都不要求玩家拥有或激活 Brewer 职业。
...
> 抽取 5 项且不重复，种子由玩家 UUID 与抽取数派生；结果写入 SavedData，登录按已存 ID 重挂。

## miningdim-brewer.toml 默认配置
...
> 酿造 2,400 tick、现实日换算、100 tick 结算间隔、12 酒槽、6,192 容量、5 层上限与 10/18/25 年阈值是编译期常量，不能通过 TOML 调整。

## Brewer 跨模块与 WebUI 联动
