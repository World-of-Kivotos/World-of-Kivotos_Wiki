...
const ids = (...paths: string[]) => paths.map((path) => 'miningdim:' + path).join('、')

export const auditPowerPressure: AuditDoc = {
  id: 'power-pressure',
  name: '能源与动态压力完整审计',
  ...
  facts: [
    ...
    { label: '物品', value: '85' },
    { label: '能源配方', value: '66' },
    { label: '阶段', value: 'P1 / P2 / P3' },
    { label: '压力档位', value: '5' },
    { label: '审计提交', value: '701093bd8492' },
  ],
  intro:
  ...
  sections: [
    {
      ...
      note: '本页统计命名空间统一为 miningdim；GameTest 类虽然位于 src/main/java 并随 JAR 入包，但只列为验证代码，不计玩家功能。',
    },
    {
      heading: '37 个方块注册总账',
      table: {
        columns: ['分类', '数量', '完整注册 ID'],
        numericCols: [1],
        monoCols: [2],
        rows: [
          ['三档发电机', '3', ids('industrial_generator', 'modern_generator', 'future_energy_generator')],
          [
            '十二级标准线缆',
            '12',
            ids(
          ...
          ['特殊线缆与控制器', '2', ids('tungsten_heat_resistant_wire', 'low_temperature_controller')],
          ['加工机器', '2', ids('metallurgic_purifier', 'air_separation_unit')],
          [
            '七矿物的普通矿与深板岩矿',
            '14',
            ids(
          ...
          ['橡胶树系', '4', ids('rubber_log', 'rubber_planks', 'rubber_leaves', 'rubber_tree_sapling')],
        ],
      },
    },
    {
    ...
    },
    {
      heading: '其余能源注册对象',
      table: {
        columns: ['注册类型', '数量', '完整 ID'],
        numericCols: [1],
        monoCols: [2],
        rows: [
          ['BlockEntityType', '7', ids('generator_controller', 'generator_port', 'energy_cable', 'low_temperature_controller', 'metallurgic_purifier', 'air_separation_unit', 'rubber_log')],
          ['MenuType', '4', ids('generator', 'low_temperature_controller', 'metallurgic_purifier', 'air_separation_unit')],
          ['RecipeType', '2', ids('metallurgic_purifying', 'air_separating')],
          ['RecipeSerializer', '2', ids('metallurgic_purifying', 'air_separating')],
          ['CreativeModeTab', '1', ids('power')],
          ['Configured feature', '8', ids('ore_bauxite', 'ore_borax', 'ore_silver', 'ore_tin', 'ore_nickel', 'ore_chromium', 'ore_tungsten', 'rubber_tree')],
          [
            'Placed feature',
            '13',
            ids(
    ...
    {
      heading: 'P1 到 P3 的可达玩法链',
      table: {
        columns: ['阶段', '核心内容', '生存推进'],
        rows: [
      ...
      note: 'P1 的三导体集合、P2 六导体和 P3 三导体由源码及对应 GameTest 明确命名；阶段内其他机器和发电机按实际配方依赖链归类。',
    },
    {
      heading: '七种矿物与矿山生成',
      table: {
        columns: ['矿物', '原料 / 锭', '矿脉大小', '入门 / 进阶 / 硬核每区块尝试'],
        numericCols: [2],
        monoCols: [1],
        rows: [
    ...
    },
    {
      heading: '橡胶与绝缘链',
      steps: [
        '丛林生物群系通过 miningdim:add_rubber_tree 注入 rubber_tree_placed，稀有度过滤为 1/24。树干高度参数为 4 + 2 + 1，树冠为半径 2、高 3 的 blob foliage。',
        '手持 rubber_tapping_knife 右击 rubber_log，获得 1 个 latex；同一原木冷却 24,000 tick，刀消耗 1 耐久，最大耐久 128。',
        '原木的割胶冷却保存在 RubberLogBlockEntity，并随精准拾取或方块物品 NBT 保留。',
        'latex 熔炼 200 tick 得 rubber；rubber 再合成五档绝缘材料。',
      ],
      table: {
        columns: ['配方', '输入', '输出'],
        monoCols: [0],
    ...
    },
    {
      heading: '三档发电机数值',
      table: {
        columns: ['发电机', '源电压', '峰值', '缓冲', '燃料芯寿命', '熔毁温度', '拒收升温 / 空载冷却'],
        numericCols: [2, 3],
        monoCols: [0],
        rows: [
    ...
    },
    {
      heading: '十三种线缆物理档案',
      table: {
        columns: ['导体或特殊线', '额定吞吐', '降效地板', '绝缘 / 耐温', '耐压', '基础线阻单位'],
        numericCols: [1, 2, 5],
        monoCols: [0],
        rows: [
      ...
      note: 'tungsten_heat_resistant_wire 名称虽然是 wire，注册对象实际是可连接电网的方块与 BlockItem，不是十二级裸导线之一。',
    },
    {
      heading: '电网结算、损耗与兼容边界',
      bullets: [
    ...
    },
    {
      heading: '加工机器与精确运行成本',
      table: {
        columns: ['工序', '输入与灌注', '耗时', '功率', '总 FE', '输出'],
        numericCols: [2, 3, 4],
        monoCols: [0],
        rows: [
    ...
    },
    {
      ...
      table: {
        ...
        monoCols: [2],
        rows: [
          ['六金属熔炼与高炉', '12', ids('raw_aluminum_to_aluminum_ingot_smelting', 'raw_aluminum_to_aluminum_ingot_blasting', 'raw_silver_to_silver_ingot_smelting', 'raw_silver_to_silver_ingot_blasting', 'raw_tin_to_tin_ingot_smelting', 'raw_tin_to_tin_ingot_blasting', 'raw_nickel_to_nickel_ingot_smelting', 'raw_nickel_to_nickel_ingot_blasting', 'raw_chromium_to_chromium_ingot_smelting', 'raw_chromium_to_chromium_ingot_blasting', 'raw_tungsten_to_tungsten_ingot_smelting', 'raw_tungsten_to_tungsten_ingot_blasting')],
          ['橡胶、工具与绝缘', '8', ids('rubber_from_latex_smelting', 'rubber_planks', 'rubber_tapping_knife', 'insulation_pvc', 'insulation_pe', 'insulation_epr', 'insulation_xlpe', 'insulation_silicone')],
          ['十二导线、十二线缆与钨特殊线', '25', ids('iron_wire', 'aluminum_wire', 'copper_wire', 'tinned_copper_wire', 'ofc_copper_wire', 'ofe_copper_wire', 'silver_plated_copper_wire', 'gold_wire', 'silver_wire', 'graphene_wire', 'nbti_superconductor_wire', 'ybco_superconductor_wire', 'iron_energy_cable', 'aluminum_energy_cable', 'copper_energy_cable', 'tinned_copper_energy_cable', 'ofc_copper_energy_cable', 'ofe_copper_energy_cable', 'silver_plated_copper_energy_cable', 'gold_energy_cable', 'silver_energy_cable', 'graphene_energy_cable', 'nbti_superconductor_energy_cable', 'ybco_superconductor_energy_cable', 'tungsten_heat_resistant_wire')],
          ['三发电机与三燃料芯', '6', ids('industrial_fuel_core', 'industrial_generator', 'modern_fuel_core', 'modern_generator', 'future_fuel_core', 'future_energy_generator')],
          ['两机器、五提纯、两空分', '9', ids('metallurgic_purifier', 'air_separation_unit', 'copper_to_deoxidized', 'deoxidized_to_ofc', 'deoxidized_to_phosphorus', 'ofc_to_ofe', 'gold_to_4n', 'air_separation_argon', 'air_separation_liquid_nitrogen')],
          ['终局材料、保险丝与控制器', '6', ids('graphene_sheet', 'superconductor_precursor', 'nbti_conductor', 'ybco_tape', 'low_temperature_controller', 'nichrome_fuse')],
        ],
      },
      bullets: [
    ...
    },
    {
      ...
      note: 'JEI 与 Jade 都是 mods.toml 中 mandatory=false 的软依赖；开发运行时只有显式传入 -PwithPowerCompat 才加载完整实现。',
    },
    {
      heading: '动态压力公式与默认配置',
      paragraphs: [
    ...
    },
    {
      heading: '三难度压力曲线',
      table: {
        columns: ['难度', 'zone 初值', '理论天花板', '默认阈值到达时间，从进入实例计'],
        numericCols: [1, 2],
        rows: [
    ...
    },
    {
      heading: '五档压力刷怪',
      table: {
        columns: ['档位', 'danger 区间', '间隔', '单波', '允许怪物', '同步视觉档'],
        numericCols: [2],
        rows: [
    ...
    {
      heading: '资源存在、测试专用与明确排除',
      table: {
        columns: ['对象', '物理状态', '本页归类'],
        rows: [
      ...
      note: 'power 的 9 个测试类为 GeneratorGameTests、GeneratorRuntimeGameTests、EnergyCableGameTests、PowerMidgameRecipeGameTests、PowerEndgameRecipeGameTests、PowerMachineGameTests、PowerMineralGameTests、RubberGameTests、LowTemperatureControllerGameTests。',
    },
    {
      heading: '关键源码与资源路径',
      bullets: [
