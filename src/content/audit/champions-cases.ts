...
export const auditChampionsCases: AuditDoc = {
  id: 'champions-cases',
  name: '冠军、战斗与武器箱',
  ...
  facts: [
    ...
    { label: '冠军词缀', value: '35/35 进入自然抽取白名单' },
    { label: '武器箱', value: '1 箱、17 皮肤、17 套显示资源' },
    { label: '启动接线', value: '冠军、战斗、开箱、附魔均由 MiningDim 装配' },
    { label: '条件依赖', value: '开箱需 TaCZ 1.1.8 与资源导出成功' },
    { label: '审计方式', value: '只读源码、注册链、资源、现有生产 JAR 与 GameTest 静态核验' },
  ],
  intro:
  ...
  sections: [
    {
      heading: '入口、门控与玩家可达性',
      table: {
        columns: ['子系统', '启动与玩家入口', '运行时条件', '审计状态'],
        rows: [
          [
            '冠军',
            'MiningDim 无条件注册 ChampionSystem；MobPressureSystem 经 ChampionSpawnSeam 自然晋升；OP 2 命令 /mchampion summon；WebUI 动作 champion.codex、champion.inspect',
            '自然生成无配置开关；命令需 OP 2；inspect 需同维度 entityId',
            '已编译、已装配、玩家可达',
          ],
          [
            '战斗减伤',
            'MiningDim 无条件注册 CombatSystem，LOWEST 阶段处理玩家受伤',
            '各减伤源自行判断职业、塔罗窗口、饮酒层数或矿脉区域',
            '已编译、已装配',
          ],
          [
            '易伤效果',
            'JobFrameworkSystem 注册 miningdim:vulnerability 与受伤处理器',
            '由撕裂、塔罗等来源施加',
            '已编译、已装配',
          ],
          [
            '金钱修补',
            'MiningDim 无条件注册 EnchantmentSystem；任务附魔书池可发放',
            '受 miningdim-money-mending.toml 与账户余额限制',
            '已编译、已装配、条件可用',
          ],
          [
            '武器箱',
            'MiningDim 无条件注册 CaseOpeningSystem；WebUI 动作 case.state、case.open、case.apply',
            'enabled=true、TaCZ 已加载、反射注册武器包成功三项同时成立',
            '已编译、条件可用',
          ],
        ],
      },
    ...
    },
    {
      heading: '星级预算与伤害基线',
      table: {
        caption: '预算来自 StarRank；“普通攻击上限”是单次对玩家最大生命值的比例。',
        columns: ['星级', '生存点', '战斗点', '机动点', '技能点', '词缀上限', '技能上限', '最高品质', '基础有效生命', '普通攻击上限'],
        numericCols: [0, 1, 2, 3, 4, 5, 6, 8],
        rows: [
    ...
    },
    {
      ...
      note: '词缀数据入口为 src/main/java/com/miningdim/champion/AffixDef.java、AffixQuality.java、AffixRoller.java 与 PointBudget.java。',
    },
    {
      heading: '35 词缀清单一：生存池 10 项',
      table: {
        columns: ['ID / 中文名', '基础成本 / 最低星级', '品质数值与真实机制', '状态'],
        monoCols: [0],
        rows: [
    ...
    {
      heading: 'Champions 与 TaCZ 联动边界',
      table: {
        columns: ['对象', 'main 中的事实', '判定'],
        rows: [
    ...
    },
    {
      ...
      note: '服务器配置文件为 miningdim-case-opening.toml；实现入口为 caseopening/CaseOpeningConfig.java、CaseWeights.java、CaseRoller.java、CaseOpeningService.java 与 CaseWebUiActions.java。',
    },
    {
      heading: '武器箱 17 款皮肤目录',
      table: {
        caption: '每款显示 ID 均为 miningdim:case_<skinId>_display。单款概率为默认权重下的精确边际概率。',
        columns: ['稀有度', 'skinId / 中文名', '限定枪械 ID', '单款概率'],
        monoCols: [1, 2],
        rows: [
    ...
    },
    {
      heading: '玩家战斗减伤',
      table: {
        caption: '多个来源按 keep=max(各来源 (1-r) 的乘积, 0.15) 合并，总减伤上限 85%。只处理玩家受伤。',
        columns: ['ReductionSource.name()', '适用伤害', '精确数值', '状态'],
        monoCols: [0],
        rows: [
      ...
      note: 'CombatSystem 当前没有中央配置，四种生产来源硬接在 PlayerDamageReduction 中；CombatConstants 仍有冠军 49% 旧注释，与当前 ChampionRedlines 的 75% 不一致。',
    },
    {
      heading: '易伤效果',
      bullets: [
      ...
      note: '源码为 effect/ModJobEffects.java、VulnerabilityEffect.java 与 VulnerabilityHurtHandler.java。没有单独效果 GameTest 文件，但冠军攻击与塔罗测试覆盖了调用链。',
    },
    {
      heading: '金钱修补附魔',
      table: {
      ...
      note: '服务器配置文件为 miningdim-money-mending.toml；源码为 enchant/MoneyMendingEnchantment.java、MoneyMendingHandler.java、RepairPricing.java，以及 quest 奖励池。',
    },
    {
      heading: '明确缺陷、半实现与未接线项',
      table: {
        columns: ['严重度', '项目', '审计结论', '源码依据'],
        rows: [
