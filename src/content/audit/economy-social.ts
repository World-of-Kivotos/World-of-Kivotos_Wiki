...
export const auditEconomySocial: AuditDoc = {
  id: 'economy-social',
  ...
  facts: [
    { label: '审计基线', value: 'main @ 701093bd8492' },
    { label: '核心货币', value: 'CREDIT / AZURE' },
    { label: '服务端动作', value: '70 个已注册 action' },
    { label: '统一数据库', value: 'miningdim.db / schema V4' },
  ...
  sections: [
    {
      heading: '一、启动装配与玩家可达性',
      table: {
        columns: ['系统', '主入口证据', '当前状态与入口'],
        monoCols: [1],
        rows: [
          [
            '统一存储',
            'src/main/java/com/miningdim/MiningDim.java:72；store/MiningStoreSubsystem.java',
            '已装配。ServerAboutToStart 打开世界根目录 miningdim.db，ServerStopped 关闭。',
          ],
          [
            '网络',
            'src/main/java/com/miningdim/MiningDim.java:76；network/NetworkSystem.java',
            '已装配。Forge SimpleChannel 承载矿区同步和双向 WebUI 消息。',
          ],
          [
            '经济',
            'src/main/java/com/miningdim/MiningDim.java:94；economy/EconomySystem.java',
            '已装配。矿物结算、钱包、登录迁移、命令及 WebUI 均有生产入口。',
          ],
          [
            '婚姻',
            'src/main/java/com/miningdim/MiningDim.java:147；marriage/MarriageSystem.java',
            '已装配。命令、戒指交互、共享背包、传送和离婚扫描均可达。',
          ],
          [
            '实体堆叠',
            'src/main/java/com/miningdim/MiningDim.java:153；stacking/StackingSystem.java',
            '已装配且默认启用。事件和周期扫描直接驱动，没有命令或 WebUI。',
          ],
          [
            'WebUI 服务端',
            'src/main/java/com/miningdim/MiningDim.java:158；webui/server/WebUiServerSubsystem.java',
            '已装配。注册网关、握手、批处理、首页面板及各业务 action。',
          ],
          [
            '跳蚤市场',
            'src/main/java/com/miningdim/MiningDim.java:168；market/MarketSubsystem.java',
            '已装配。仅通过 WebUI action 操作，无聊天命令。',
          ],
          [
            '任务',
            'src/main/java/com/miningdim/MiningDim.java:173；quest/QuestSystem.java',
            '已装配且默认启用。事件、命令、WebUI 和世界 SavedData 均接通。',
          ],
          [
            'WebUI 客户端',
            'src/main/java/com/miningdim/MiningDim.java:182；client/webui/WebUiClientSubsystem.java',
            '客户端装配，但实际浏览器依赖可选 MCEF；缺少 MCEF 时 Java 业务仍在，平板 UI 不可用。',
          ],
        ],
      },
      note:
    ...
    },
    {
      heading: '二、货币模型与完整流向',
      table: {
        columns: ['路径', '货币变化', '闸门与含义'],
        rows: [
          [
            '矿区高价值矿物破坏',
            '生成 CREDIT',
            '仅矿业维度、有效实例区域、未取消的 BreakEvent；钻石、金、下界合金碎片按定价与两层衰减结算。',
          ],
          [
            '农夫出售小麦',
            '生成 CREDIT',
            '基础单价 1，与卖矿、精英怪奖励、特工奖励共用 credit_faucet。',
          ],
          [
            '精英怪与特工奖励',
            '生成 CREDIT / AZURE',
            'CREDIT 走共享 credit_faucet；两条 AZURE 来源都走共享 azure_faucet 与同一每日 30 上限。',
          ],
          [
            '任务领奖',
            '生成 CREDIT',
            '使用独立 quest_faucet，不占卖矿、卖菜和战斗奖励的 credit_faucet 档位。',
          ],
          [
            '跳蚤市场成交',
            '买家向卖家转移 CREDIT',
            '不是 faucet。在线卖家即时入账，离线卖家写 pending_payout 后于登录领取。',
          ],
          [
            '系统消费',
            '销毁 CREDIT 或 AZURE',
            '市场上架费、任务刷新、婚姻、卡包、开箱、金钱修补、调味台和军械工作费都是生产 sink。',
          ],
          [
            '管理员调整',
            '注入或销毁',
            '/economy grant 与 admin.economy.set 绕过自然产出循环，属于运维入口并写审计日志。',
          ],
        ],
      },
      bullets: [
    ...
    {
      heading: '三、基础价格、衰减和每日上限',
      table: {
        columns: ['项目', '默认值', '计算或时钟'],
        numericCols: [1],
        rows: [
    ...
    {
      heading: '四、生产 sink 与默认收费',
      table: {
        columns: ['消费点', '默认收费', '可达性或备注'],
        numericCols: [1],
        rows: [
    ...
    {
      heading: '五、经济命令、WebUI 与未接通防滥用代码',
      table: {
        columns: ['入口或定义', '权限与行为', '判定'],
        monoCols: [0],
        rows: [
          [
            '/economy grant <target> <credit>=1+ <azure>=1+',
            '权限等级 2；CREDIT 与 AZURE 原子 bundle 发放并记录审计日志。',
            '已接线的运维入口。',
          ],
          ...
          [
            'admin.economy.set',
            '仅 OP；把 CREDIT 或 AZURE 设为 0 至 9,007,199,254,740,991，按差额调用账本。',
            '已注册。',
          ],
    ...
    {
      heading: '六、跳蚤市场的上架与成交闭环',
      steps: [
        '卖家调用 market.place，提交背包槽、数量、单价和 CREDIT；服务端复核数量、物品、塔罗限制与 P2P 日上限。',
        '服务端以基础价值和挂牌单价计算上架费，先从卖家钱包销毁费用，再序列化 ItemStack 并插入 ACTIVE 托管行，最后缩减玩家背包。',
        '买家调用 market.buy；不能购买自己的订单，count 小于等于 0 表示全买，也可以部分成交。',
        'SQLite 事务内完成买家扣款、订单数量或 SOLD 状态、成交记录，以及在线卖家入账或离线 pending_payout。',
        '事务提交后把物品放入买家背包；背包放不下时掉在脚边。卖家下次登录时事务性领取离线款。',
        '卖家可对自己的 ACTIVE 订单调用 market.cancel，托管物品返还背包或掉落；上架费不退。',
      ],
      bullets: [
    ...
    },
    {
      ...
      table: {
        ...
        rows: [
          [
            '上架费公式',
            'round(max(V0, VR) × count × (0.20 + 0.04 × ln(VR / V0)²))；锚值最低按 1，无上限。',
            'market/MarketFee.java；MarketConstants.java',
          ],
          ...
          [
            'V0 解析',
            '管理员覆盖优先，其次仅有上述四个默认值；没有设计文档曾提到的成交中位数层。',
            'market/BaseValueResolver.java',
          ],
          [
            '铜铁 P2P 日上限',
            '512 件；ACTIVE 挂单总量加服务器本地日期内 SOLD 数量。',
            'market/MarketConstants.java；MarketEngine.java:523-537',
          ],
          [
            '受限铜铁 ID',
            'copper_ore、raw_copper、copper_ingot、iron_ore、raw_iron、iron_ingot。',
            'market/MarketConstants.java',
          ],
          [
            '塔罗',
            '只允许品质 R；SR、SSR、UR、Shiny 以及 NBT 畸形卡均拒绝。',
            'market/MarketTradeWhitelist.java',
          ],
          [
            '容器递归',
            '检查 BlockEntityTag.Items 一层，阻止在容器内夹带禁售塔罗。',
            'market/MarketTradeWhitelist.java',
          ],
          [
            '其他物品',
            '默认允许；婚戒和其他带绑定 NBT 的装备没有被市场白名单显式阻止。',
            'market/MarketTradeWhitelist.java',
          ],
          [
            '分类树',
            '顶层 ores、weapons、ammo、gear、food、other；ores 下有 ore、ingot、gem。',
            'market/MarketCategoryTree.java',
          ],
        ],
      },
      note:
    ...
    {
      heading: '九、市场已确认缺陷与原子性边界',
      table: {
        columns: ['严重度', '缺陷', '实际后果'],
        rows: [
          [
            'Major',
            '极低总值的挂牌费 round 后为 0，但账本 tryCharge 要求正数。',
            '总价 1 或 2、且 20% 基础费四舍五入为 0 的最便宜挂单会以 ILLEGAL_AMOUNT 失败。',
          ],
          [
            'Major',
            '上架费扣款、listing 插入和背包 shrink 不在一个事务。',
            '插入失败可能只损失费用；插入成功后到缩减背包前硬崩溃存在复制窗口。',
          ],
          [
            'Major',
            '购买事务先提交，之后才交付物品。',
            '普通背包失败会改为脚边掉落，但提交后、交付前硬崩溃仍可能让买家付钱却丢物。',
          ],
          [
            'Major',
            'Java 新合同把 market.categories 改成只返回带 leafCount 的分支骨架。',
            '真实叶项改由 market.categoryItems 分页返回，但 webui/src/lib/actions.ts 未声明该动作，BrowsePage 仍按旧递归叶结构渲染，分类分支没有可选叶。',
          ],
          [
            'Minor',
            '握手兼容检查只把“前端声明但服务端缺少”视为不兼容。',
            'market.categoryItems 是服务端额外动作，只写 console 信息，不会阻止旧前端继续启动，因此合同漂移被放过。',
          ],
        ],
      },
      note:
    ...
    },
    {
      heading: '十二、内置日常任务池',
      table: {
        columns: ['池', '任务 ID 与精确目标', 'difficulty'],
        monoCols: [1],
        rows: [
    ...
    },
    {
      heading: '十四、任务物品奖励池',
      table: {
        columns: ['奖励池', '物品、数量与权重'],
        rows: [
          [
            '日常与特殊：资源',
            '煤 8-16 w12；铜锭 6-12 w12；铁锭 4-8 w10；小麦、胡萝卜、马铃薯各 12-24 w10；甜菜根 8-16 w8；南瓜 4-8 w6；西瓜片 8-16 w6。',
          ],
          [
            '日常与特殊：装备',
            '铁镐、铁剑各 1 w4；铁锹、铁头盔、铁靴各 1 w3；铁胸甲、铁护腿各 1 w2。',
          ],
          [
            '周常与隐藏：资源',
            '金锭 8-16 w12；红石 16-32 w12；青金石 12-24 w10；绿宝石 4-8 w8；钻石 2-5 w6；下界疣 8-16 w8；可可豆、甜浆果各 12-24 w8；闪烁的西瓜片 4-8 w6。',
          ],
          [
            '周常与隐藏：装备',
            '钻石镐、钻石剑各 1 w4；钻石锹、头盔、护腿、靴各 1 w3；钻石胸甲 1 w2。',
          ],
          [
            '附魔书常见',
            '经验修补 I w15；金钱修补 I w15。',
          ],
          [
            '附魔书稀有',
            '时运 III w3；抢夺 III w3。',
          ...
          [
            '附魔书其余',
            '效率 IV、耐久 III、精准采集 I、保护 IV、摔落缓冲 IV、爆炸保护 IV、火焰保护 IV、弹射物保护 IV、水下速掘 I、水下呼吸 III、深海探索者 III、锋利 IV、荆棘 III、迅捷潜行 III，均 w6。',
          ],
        ],
      },
      bullets: [
      ...
      note: '证据：quest/QuestItemRewards.java。w 表示该条目在对应权重池中的整数权重。',
    },
    {
      heading: '十五、任务操作、进度边界与缺陷',
      table: {
        columns: ['主题', '当前实现'],
        rows: [
      ...
      note: '证据：quest/QuestService.java:107-113、QuestEventHooks.java:54-68、QuestTaczHooks.java、QuestPlacedBlocks.java。',
    },
    {
      heading: '十六、婚姻配置与生命周期',
      table: {
        columns: ['阶段', '默认规则', '入口或状态'],
        rows: [
    ...
    {
      heading: '十八、离婚、财产返还与已确认缺陷',
      steps: [
        '任一方通过 /marriage divorce 或 marriage.divorce 发起，发起者支付默认 10,000 CREDIT；重复发起保持幂等。',
        '进入 24 小时 server gameTime 托管期，待离婚期间共享背包被冻结且现有容器会关闭。',
        '发起者可用 /marriage divorce cancel 取消并退款；非发起配偶可用 /marriage divorce confirm 提前确认。WebUI 只支持发起，不支持取消或确认。',
        '系统每 100 tick 扫描到期记录并完成离婚。共享背包按槽位 depositor 返还；未知归属采用偶数槽给 A、奇数槽给 B，背包满则形成可持久领取物或掉落。',
        '双方 divorceCount 先加一，再按 7 天 × (1 + 新 divorceCount) 计算再婚冷却；因此第一次离婚后的实际冷却是 14 个服务器运行日，第二次是 21 日。',
      ],
      bullets: [
      ...
      note: '证据：marriage/MarriageDivorce.java、MarriageSystem.java、MarriageTuning.java、MarriageHistory.java。',
    },
    {
      heading: '十九、婚姻 action 与状态接口',
      table: {
        columns: ['动作 ID', '用途', '边界'],
        monoCols: [0],
        rows: [
    ...
    },
    {
      ...
      note: '真源：stacking/StackingConfig.java、StackingSystem.java、StackMerge.java、StackMatchKey.java。',
    },
    {
      heading: '二十一、堆叠实体死亡、交互与缺陷',
      table: {
        columns: ['场景', '整堆行为'],
        rows: [
      ...
      note: '堆大小和宽限写在实体 Forge persistent data：miningdim:StackSize、miningdim:StackNoMergeUntil；羊另有 miningdim:ShearRegrowPending。',
    },
    {
      heading: '二十二、统一 SQLite 生命周期与 schema V4',
      table: {
        columns: ['对象', '结构或策略', '职责'],
        monoCols: [0],
        rows: [
      ...
      note: '证据：store/MiningDb.java、MiningStore.java、MiningSchema.java、LegacyStoreImport.java、economy/EconomyLedgerBootstrap.java。',
    },
    {
      heading: '二十三、SQLite 表清单',
      table: {
        columns: ['版本', '表', '关键字段与用途'],
        monoCols: [1, 2],
        rows: [
    ...
    {
      heading: '二十四、非 SQLite 持久化矩阵',
      table: {
        columns: ['载体', '键或对象', '持久内容'],
        monoCols: [1],
        rows: [
    ...
    },
    {
      ...
      note: '证据：network/MiningNetwork.java、C2SWebUiRequest.java、webui/server/WebUiServerDispatcher.java、WebUiRateLimiter.java、WebUiBatchAction.java。',
    },
    {
      heading: '二十六、服务端 WebUI action 全量分组',
      table: {
        columns: ['域', '已注册动作 ID'],
        monoCols: [1],
        rows: [
    ...
    },
    {
      ...
      note: '证据：client/webui/WebUiClient.java、WebUiBridge.java、WebUiScreen.java、config/MiningClientConfig.java、webui/src/lib/actions.ts、build.gradle。',
    },
    {
      heading: '二十八、死代码、测试代码与外部边界总表',
      table: {
        columns: ['对象', '物理状态', '审计结论'],
        rows: [
